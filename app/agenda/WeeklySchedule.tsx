...
export function WeeklySchedule({ professionals, schedule = weeklySchedule }: { professionals: Professional[]; schedule?: Schedule }) {
  ...
  return <section className="services section weekly-section" id="atendimentos" aria-labelledby="weekly-title">
    ...
    <div className="weekly-days">{days.map(day => <article className={'weekly-day' + (!day.people.length ? ' weekly-empty' : '')} key={day.iso} aria-labelledby={'day-' + day.iso}>
      ...
      {day.people.length > 0 ? <ul className="weekly-people">{day.people.map(person => <li key={person.id}><figure className="weekly-person"><div className="weekly-photo"><img src={person.photo ?? `/profissionais/${person.id}.webp`} alt={person.name} width="150" height="130" loading="lazy"/></div><figcaption><h4>{person.name}{person.note && <> ({person.note})</>}</h4>{person.area && <p>{person.area}</p>}</figcaption></figure></li>)}</ul> : <p className="weekly-pending">Programação a confirmar.<span>Consulte a equipe para informações sobre este dia.</span></p>}
    </article>)}</div>
